function validateRequire (val: string) {
...
function validateWithoutSpecialSymbol (string: string) {
  /* eslint-disable no-useless-escape */
  return {result: !new RegExp(/[ `!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?~]/g).test(string), message: 'Не должно быть спецсимволов' }
}

function validateLength (string: string) {
...
function validateOnceLetterCapitalize (string: string) {
